...
async function showPerformanceMenu() {
  ...
  switch (action) {
    case 'stats':
      ...
      break;
    case 'connections':
      await showPerformanceConnectionStatus();
      break;
    case 'cache':
...
async function showPerformanceConnectionStatus() {
  console.clear();
...
// Function to display a single trending token
export function displayTrendingToken(index, trendingData) {
  if (!trendingData || index >= trendingData.length) {
    console.log(`${colors.red}❌ Invalid trending token index${colors.reset}`);
    return;
  ...
  console.clear();
  console.log(`${colors.cyan}📊 Trending Token ${index + 1}/${trendingData.length}${colors.reset}`);
  console.log(`${colors.white}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}`);
  
  // Token name and momentum
  const momentumColor = m.momentum >= 0 ? colors.green : colors.red;
  const momentumSymbol = m.momentum >= 0 ? '🚀' : '📉';
  console.log(`${colors.yellow}${t.token.Symbol}${colors.reset} ${momentumColor}${momentumSymbol} ${m.momentum.toFixed(2)}%${colors.reset}`);
  
  // Token address
  console.log(`${colors.blue}📍 Address: ${t.token.MintAddress}${colors.reset}`);
  
  // Price and changes
  const priceChangeColor = m.priceChange >= 0 ? colors.green : colors.red;
  const priceChangeSymbol = m.priceChange >= 0 ? '↗' : '↘';
  console.log(`💰 Price: $${m.currentPrice.toFixed(6)} ${priceChangeColor}${priceChangeSymbol} ${m.priceChange.toFixed(2)}%${colors.reset}`);
  
  // Volume metrics
  const volumeChangeColor = m.volumeChange >= 0 ? colors.green : colors.red;
  const volumeChangeSymbol = m.volumeChange >= 0 ? '↗' : '↘';
  console.log(`📊 Volume (5m): $${m.recentVolume.toLocaleString()} ${volumeChangeColor}${volumeChangeSymbol} ${m.volumeChange.toFixed(2)}%${colors.reset}`);
  
  // Trade frequency
  const freqChangeColor = m.freqChange >= 0 ? colors.green : colors.red;
  const freqChangeSymbol = m.freqChange >= 0 ? '↗' : '↘';
  console.log(`⚡ Trades/min: ${m.tradeFreq.toFixed(1)} ${freqChangeColor}${freqChangeSymbol} ${m.freqChange.toFixed(2)}%${colors.reset}`);
  ...
  if (j) {
    ...
    // Market cap and liquidity
    if (j.marketCap) {
      console.log(`🏦 MCap: $${j.marketCap.toLocaleString()}`);
    }
    if (j.liquidity) {
      const liquidityColor = j.liquidityScore >= 75 ? colors.green :
                           j.liquidityScore >= 50 ? colors.yellow :
                           colors.red;
      console.log(`💧 Liq: ${liquidityColor}$${j.liquidity.toLocaleString()}${colors.reset} (Score: ${j.liquidityScore})`);
    }
    ...
    if (j.priceChange24h !== undefined) {
      const price24hColor = j.priceChange24h >= 0 ? colors.green : colors.red;
      const price24hSymbol = j.priceChange24h >= 0 ? '↗' : '↘';
      console.log(`📈 24h Price: ${price24hColor}${price24hSymbol} ${j.priceChange24h.toFixed(2)}%${colors.reset}`);
    }
    if (j.volumeChange24h !== undefined) {
      const vol24hColor = j.volumeChange24h >= 0 ? colors.green : colors.red;
      const vol24hSymbol = j.volumeChange24h >= 0 ? '↗' : '↘';
      console.log(`📊 24h Volume: ${vol24hColor}${vol24hSymbol} ${j.volumeChange24h.toFixed(2)}%${colors.reset}`);
    }
    
    // Holders
    if (j.holderCount) {
      console.log(`👥 Holders: ${j.holderCount.toLocaleString()}`);
    }
  }
  
  console.log(`${colors.white}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}`);
  console.log(`${colors.cyan}Navigation: W (Next) | S (Previous) | R (Menu) | Q (Exit)${colors.reset}`);
}
  ...
  try {
    ...
    if (menuState === MENU_STATES.MONITOR) {
      ...
      const modeInfo = {
        ...
        description: [MODE_DESCRIPTIONS[queryType] || 'Mode description not available']
      };
...
function cleanup() {
  ...
  // Cleanup volume tracker
  if (volumeTracker) {
    volumeTracker.cleanup();
  }
...
async function startStream(queryType = 'pump') {
  try {
    ...
    } else if (queryType === 'min500tx') {
      queryConfig = tokensMin500TxQuery;
    } else if (queryType === 'trending') {
      queryConfig = trendingGainersQuery;
    } else {
    ...
    if (queryType === 'graduated') {
      ...
      // For trending mode, we need to handle the special structure
      if (!result?.data?.Solana?.recentTrades) {
        ...
        result.data.Solana.recentTrades = fallbackResult.data.Solana.DEXTrades;
        result.data.Solana.previousTrades = [];
      }
    }
    ...
    if (queryType === 'trending') {
      trades = result.data.Solana.recentTrades || [];
    } else {
      trades = result.data.Solana[dataKey];
    }
    ...
    // Special filtering for min500tx mode
    if (queryType === 'min500tx' && validTrades.length > 0) {
      console.log(`${colors.cyan}🔍 Filtering tokens with minimum 500 transactions...${colors.reset}`);
      ...
      validTrades.forEach(trade => {
        ...
        if (mintAddress) {
          ...
          tokenGroups.get(mintAddress).push(trade);
        }
      });
      
      // Filter tokens with at least 500 transactions and track volume
      const highActivityTokens = [];
      ...
      for (const [mintAddress, trades] of tokenGroups) {
        if (trades.length >= 500) {
          ...
          trades.forEach(trade => {
            const tradeVolume = trade.Trade?.Buy?.AmountInUSD || trade.Trade?.Sell?.AmountInUSD || 0;
            if (tradeVolume > 0) {
          ...
          // Take the most recent trade for each high-activity token
          const mostRecentTrade = trades.sort((a, b) => 
            new Date(b.Block.Time) - new Date(a.Block.Time)
          )[0];
          
          highActivityTokens.push(mostRecentTrade);
          
          // Store volume statistics
          volumeStats.set(mintAddress, {
            totalVolume,
            tradeCount: trades.length,
            volumeData,
            avgVolume: totalVolume / trades.length
          });
          
          // Add to volume tracker for real-time tracking
          if (totalVolume > 0) {
            volumeTracker.addVolumeData(mintAddress, totalVolume, Date.now());
          }
        }
      }
      
      if (highActivityTokens.length > 0) {
        console.log(`${colors.green}✅ Found ${highActivityTokens.length} tokens with 500+ transactions${colors.reset}`);
        validTrades = highActivityTokens;
      } else {
        console.log(`${colors.yellow}⚠️ No tokens found with 500+ transactions in the last 60 minutes${colors.reset}`);
        console.log(`${colors.cyan}💡 Try reducing the threshold or increasing the time window${colors.reset}`);
      }
    }
    
    // Process trending mode trades
    if (queryType === 'trending' && validTrades.length > 0) {
      console.log(`${colors.magenta}🔥 Processing trending tokens...${colors.reset}`);
      
      // Get recent and previous trades
      const recentTrades = result.data.Solana.recentTrades;
      const previousTrades = result.data.Solana.previousTrades;
      ...
      for (const [mintAddress, data] of tokenGroups) {
        ...
        const recentFreq = recent.length / 5; // trades per minute in last 5 min
        const previousFreq = previous.length / 5; // trades per minute in previous 5 min
        const freqChange = previousFreq > 0 ? ((recentFreq - previousFreq) / previousFreq) * 100 : 0;
        
        // Get Jupiter API data
        let jupiterData = null;
        try {
          const jupiterResponse = await checkJupiterTokenRealtime(mintAddress);
          if (jupiterResponse.success && jupiterResponse.data) {
            jupiterData = jupiterResponse.data;
          }
        } catch (error) {
          // Silent fail, we'll use BitQuery data if Jupiter fails
        }
        
        // Calculate momentum score with Jupiter data if available
        const momentum = (
          (volumeChange * 0.3) + // 30% weight to volume change
          (priceChange * 0.3) + // 30% weight to price change
          (freqChange * 0.2) + // 20% weight to frequency change
          ((jupiterData?.priceChange24h || 0) * 0.1) + // 10% weight to 24h price change
          ((jupiterData?.volumeChange24h || 0) * 0.1) // 10% weight to 24h volume change
        );
        
        // Calculate liquidity score if available
        let liquidityScore = 0;
        if (jupiterData?.liquidity && jupiterData?.marketCap) {
          const liquidityRatio = (jupiterData.liquidity / jupiterData.marketCap) * 100;
          liquidityScore = liquidityRatio > 10 ? 100 : 
                          liquidityRatio > 5 ? 75 :
                          liquidityRatio > 2 ? 50 :
                          liquidityRatio > 1 ? 25 : 0;
        }
        ...
        tokenMetrics.push({
          ...
          metrics: {
            ...
            jupiter: jupiterData ? {
              ...
              volumeChange24h: jupiterData.volumeChange24h,
      ...
      tokenMetrics.sort((a, b) => b.metrics.momentum - a.metrics.momentum);
              ...
              // Take top 10 tokens and store for pagination
        const trendingTokens = tokenMetrics.slice(0, 10).map(t => t.recentTrade);
        const trendingTokenData = tokenMetrics.slice(0, 10); // Store full data for display
        
        if (trendingTokens.length > 0) {
          console.log(`${colors.green}✅ Found ${trendingTokens.length} trending tokens${colors.reset}`);
          
          // Store trending data for pagination
          appState.setTrendingData(trendingTokenData);
          appState.setCurrentTrendingIndex(0);
          
          // Display first token
          displayTrendingToken(0, trendingTokenData);
          
          validTrades = trendingTokens;
        } else {
          console.log(`${colors.yellow}⚠️ No trending tokens found${colors.reset}`);
        }
    }
...
// Volume tracking for min500tx mode
const volumeTracker = {
  data: new Map(),
  
  // Add volume data for a token
  addVolumeData(tokenMint, volume, timestamp) {
    if (!this.data.has(tokenMint)) {
      this.data.set(tokenMint, []);
    }
    
    const tokenData = this.data.get(tokenMint);
    tokenData.push({ volume, timestamp });
  ...
  // Get volume change for a token
  getVolumeChange(tokenMint, timeWindow = 60) { // 60 minutes default
    const tokenData = this.data.get(tokenMint);
    ...
    const recentData = tokenData.filter(entry => entry.timestamp >= windowStart);
    ...
    const oldestVolume = recentData[0].volume;
    const newestVolume = recentData[recentData.length - 1].volume;
    const volumeChange = ((newestVolume - oldestVolume) / oldestVolume) * 100;
    
    return {
      change: volumeChange,
      oldVolume: oldestVolume,
      newVolume: newestVolume,
      dataPoints: recentData.length
    };
  },
  
  // Get volume trend for a token
  getVolumeTrend(tokenMint, timeWindow = 60) {
    const tokenData = this.data.get(tokenMint);
    ...
    for (let i = 1; i < recentData.length; i++) {
      if (recentData[i].volume > recentData[i-1].volume) {
        increasingCount++;
      } else if (recentData[i].volume < recentData[i-1].volume) {
        decreasingCount++;
      }
    }
    
    const totalComparisons = recentData.length - 1;
    const trendStrength = ((increasingCount - decreasingCount) / totalComparisons) * 100;
  ...
  cleanup() {
    ...
    for (const [tokenMint, tokenData] of this.data) {
      const filteredData = tokenData.filter(entry => (now - entry.timestamp) < maxAge);
      if (filteredData.length === 0) {
        this.data.delete(tokenMint);
      } else {
        this.data.set(tokenMint, filteredData);
      }
    }
  }
};
...
async function processTrade(trade, index) {
  try {
    ...
    // Enhanced Jupiter Token Information (with volume tracking for min500tx mode)
        ...
        if (jupiterData.success && jupiterData.data) {
          ...
          if (data.holderCount) {
            console.log(`${colors.orange}👥 Holders: ${data.holderCount.toLocaleString()}${colors.reset}`);
                      ...
                      // Volume tracking for min500tx mode
            if (appState.getCurrentMode() === 'min500tx') {
              console.log(`${colors.magenta}📊 Volume Analysis:${colors.reset}`);
              
              // Get volume data from trade
              const tradeVolume = trade.Trade?.Buy?.AmountInUSD || trade.Trade?.Sell?.AmountInUSD || 0;
              if (tradeVolume > 0) {
                console.log(`${colors.cyan}📈 Current Trade Volume: $${tradeVolume.toFixed(2)}${colors.reset}`);
                
                // Add to volume tracker
                volumeTracker.addVolumeData(buyToken.MintAddress, tradeVolume, Date.now());
              }
              
              // Get volume change from tracker
              const volumeChange = volumeTracker.getVolumeChange(buyToken.MintAddress, 60); // 60 minutes
              if (volumeChange) {
                const changeColor = volumeChange.change >= 0 ? colors.green : colors.red;
                const changeSymbol = volumeChange.change >= 0 ? '↗' : '↘';
                console.log(`${changeColor}📊 Volume Change (60m): ${changeSymbol} ${volumeChange.change.toFixed(2)}%${colors.reset}`);
                console.log(`${colors.blue}📈 Data Points: ${volumeChange.dataPoints}${colors.reset}`);
              }
              
              // Get volume trend
              const volumeTrend = volumeTracker.getVolumeTrend(buyToken.MintAddress, 60);
              if (volumeTrend) {
                const trendColor = volumeTrend.trend >= 0 ? colors.green : colors.red;
                const trendSymbol = volumeTrend.trend >= 0 ? '↗' : '↘';
                console.log(`${trendColor}📈 Volume Trend (60m): ${trendSymbol} ${volumeTrend.trend.toFixed(1)}%${colors.reset}`);
                console.log(`${colors.cyan}📊 Trend Details: ${volumeTrend.increasing}↑ ${volumeTrend.decreasing}↓${colors.reset}`);
              }
              
              // Calculate volume change if we have historical data
              if (data.volume24h) {
                const volumeImpact = ((tradeVolume / data.volume24h) * 100).toFixed(2);
                console.log(`${colors.blue}📊 24h Volume: $${data.volume24h.toLocaleString()}${colors.reset}`);
                console.log(`${colors.yellow}📈 Volume Impact: ${volumeImpact}% of 24h volume${colors.reset}`);
              }
              
              // Volume trend analysis from Jupiter data
              if (data.volumeChange24h !== undefined) {
                const volumeColor = data.volumeChange24h >= 0 ? colors.green : colors.red;
                const volumeSymbol = data.volumeChange24h >= 0 ? '↗' : '↘';
                console.log(`${volumeColor}📊 Volume 24h Change: ${volumeSymbol} ${data.volumeChange24h.toFixed(2)}%${colors.reset}`);
              }
              
              // Price change analysis
              if (data.priceChange24h !== undefined) {
                const priceColor = data.priceChange24h >= 0 ? colors.green : colors.red;
                const priceSymbol = data.priceChange24h >= 0 ? '↗' : '↘';
                console.log(`${priceColor}💰 Price 24h Change: ${priceSymbol} ${data.priceChange24h.toFixed(2)}%${colors.reset}`);
              }
              
              // Liquidity analysis
              if (data.liquidity && data.marketCap) {
                const liquidityRatio = (data.liquidity / data.marketCap * 100).toFixed(2);
                const liquidityColor = parseFloat(liquidityRatio) > 10 ? colors.green : 
                                     parseFloat(liquidityRatio) > 5 ? colors.yellow : colors.red;
                console.log(`${liquidityColor}💧 Liquidity/MCap Ratio: ${liquidityRatio}%${colors.reset}`);
              }
              
              // Market activity indicators
              if (data.holderCount && data.volume24h) {
                const avgVolumePerHolder = (data.volume24h / data.holderCount).toFixed(2);
                console.log(`${colors.orange}👥 Avg Volume/Holder: $${avgVolumePerHolder}${colors.reset}`);
              }
              
              // Volume momentum indicator
              if (volumeChange && volumeTrend) {
                const momentum = (volumeChange.change + volumeTrend.trend) / 2;
                const momentumColor = momentum >= 0 ? colors.green : colors.red;
                const momentumSymbol = momentum >= 0 ? '🚀' : '📉';
                console.log(`${momentumColor}${momentumSymbol} Volume Momentum: ${momentum.toFixed(1)}%${colors.reset}`);
              }
...
async function handleSellToken(wallet) {
  ...
  try {
    ...
    try {
      ...
      // Show realized output using executed result if available, otherwise fall back to quote
      try {
        let soldValueLamports = 0;
        if (result && result.quote && typeof result.quote.outAmount === 'number') {
          soldValueLamports = result.quote.outAmount;
        } else if (quote && typeof quote.outAmount === 'number') {
          soldValueLamports = quote.outAmount;
        }
        const soldValue = soldValueLamports / LAMPORTS_PER_SOL;
        console.log(`${colors.green}💰 Sold for: ${soldValue.toFixed(4)} SOL${colors.reset}`);

        // Calculate profit percentage based on original purchase (if available)
        const profitInfo = await calculateAndDisplayProfit(tokenMint, amountToSell, tokenDecimals);
        if (profitInfo.percentage !== 0) {
          console.log(`${profitInfo.symbol} ${profitInfo.color}Profit: ${profitInfo.percentage.toFixed(2)}%${colors.reset}`);
        }
      } catch (error) {
